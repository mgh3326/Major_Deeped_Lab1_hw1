...
END_MESSAGE_MAP()
// CViewerDlg dialog
CViewerDlg::CViewerDlg(CWnd* pParent
	/*=nullptr*/
)
...
void CViewerDlg::DisplayBitmap(CDC* pDC, CRect rect, IplImage* DispIplImage)
{
	...
	if (DispIplImage->nChannels == 3)
	{
		...
		for (int h = 0; h < DispIplImage->height; h++)
		{
			...
			{
				ReverseImg->imageData[h * DispIplImage->widthStep + w * 3] = DispIplImage->imageData[(DispIplImage->
					height - h) * DispIplImage->widthStep + w * 3];
				ReverseImg->imageData[h * DispIplImage->widthStep + w * 3 + 1] = DispIplImage->imageData[(DispIplImage->
					height - h) * DispIplImage->widthStep + w * 3 + 1];
				ReverseImg->imageData[h * DispIplImage->widthStep + w * 3 + 2] = DispIplImage->imageData[(DispIplImage->
					height - h) * DispIplImage->widthStep + w * 3 + 2];
			}
		}
		bitmapInfo.bmiHeader.biBitCount = DispIplImage->depth * DispIplImage->nChannels;
		pDC->SetStretchBltMode(COLORONCOLOR);
		::StretchDIBits(pDC->GetSafeHdc(), rect.left, rect.top, rect.right, rect.bottom, 0, 0, DispIplImage->width,
			DispIplImage->height, ReverseImg->imageData, &bitmapInfo, DIB_RGB_COLORS, SRCCOPY);
	...
	else
	{
		...
		for (int h = 0; h < DispIplImage->height; h++)
		{
			for (int w = 0; w < DispIplImage->width; w++)
			{
				ReverseImg->imageData[h * DispIplImage->widthStep + w] = DispIplImage->imageData[(DispIplImage->height -
					h) * DispIplImage->widthStep + w];
			}
		}
		...
		pDC->SetStretchBltMode(COLORONCOLOR);
		::StretchDIBits(pDC->GetSafeHdc(), rect.left, rect.top, rect.right, rect.bottom, 0, 0, tmp->width, tmp->height,
			tmp->imageData, &bitmapInfo, DIB_RGB_COLORS, SRCCOPY);
		cvReleaseImage(&ReverseImg);
		cvReleaseImage(&tmp);
	}
}
...

{
	...
	{
		if (dlg.GetFileExt() != "jpg" && dlg.GetFileExt() != "JPG" && dlg.GetFileExt() != "raw" && dlg.GetFileExt() !=
			"yuv" && dlg.GetFileExt() != "RAW")
...

				{
...

					{
...

						{
							...
							Ori_Img.at<Vec3b>(h, w)[1] = ch_in_1[h][w] - 0.344 * (ch_in_2[h][w] - 128) - 0.714 * (
								ch_in_3[h][w] - 128);
...

				{
...

					{
...

						{
							...
							Ori_Img.at<Vec3b>(h, w)[1] = ch_in_1[h][w] - 0.344 * (ch_in_2[h / 2][w / 2] - 128) - 0.714 *
								(ch_in_3[h / 2][w / 2] - 128);
...

	{
...

		{
...

			{
				...
				{
					ch_in_gray[h][w] = saturate_cast<unsigned char>(
						0.299 * ch_in_1[h][w] + 0.587 * ch_in_2[h][w] + 0.114 * ch_in_3[h][w]);
				}
			}
...

{
	...
	else
	{
		...
		result_pixel = (h_length_down * (w_length_right * left_up + w_length_left * right_up)) + (h_length_up * (
			w_length_right * left_down + w_length_left * right_down));
...

{
	...
	else
	{
...

		{
			...
			{
				first_cal[i] = inverse_matrix[i][0] * In[(h1 - 1) + j][w1 - 1] + inverse_matrix[i][1] * In[(h1 - 1) + j]
					[w1] +
			...
			mid_cal[j] = first_cal[0] * (n_w * n_w * n_w) + first_cal[1] * (n_w * n_w) + first_cal[2] * (n_w)+
				first_cal[3];
...

{
...

	{
		...
		Mat MatchImg(256, 256, CV_8UC1);
		Mat RefImg(nHeight_in, nWidth_in, CV_8UC1);
...

		{
...

			{
...

				{
					...
					MessageBox("FilterSize가 잘못되었습니다. FilterSize는 3 이상의 정수 값을 가져야 합니다.");
			}
